...
public class MainFrame extends JFrame {
    ...
    private void selectedTableItem(FileListItemContainer fval) {
        if (fval != null) {
            if (fval.getFile() != null && !fval.getFile().isDirectory()) {
                ImageIcon ii = null;
                String contentType = Mime.getMimeTypeFile(fval.getFile().getName());
                if (contentType != null && contentType.startsWith("image")) {
                    ii = new ImageIcon(fval.getFile().getPath());
                }
    ...
    private void createFileTable(File imageMagickPath) {
        ...
        final FileTableModel tableModel = new FileTableModel(new File("D:/pub"), 4, null);
